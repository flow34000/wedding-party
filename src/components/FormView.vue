      ...
      <form novalidate @submit.prevent="submitRSVP1" class="flex flex-column gap-2" v-if="step === 1">
        ...
        <div class="formgrid grid" v-if="confirm === true">
          ...
          <div class="field col-12" v-if="nbChildren > 0">
            <InputText
              id="ageOfChildren"
              v-model="ageOfChildren"
              type="text"
              :class="[{ 'p-invalid': errorMessage }, 'w-full']"
              aria-describedby="text-error"
              size="small"
              placeholder="Age des enfants (8,12)"
            />
      ...
      <form novalidate @submit.prevent="submitRSVP2" class="flex flex-column gap-2" v-if="step === 2">
        <div class="formgrid grid">
          <template v-if="nbChildren > 0">
            <div class="field col-12">
              <label
                >Un menu spécial pour les enfants de 3 à 12 ans est prévu. En deça de 3 ans nous demandons aux parents de fournir le repas
            ...
            <div class="field col-12 md:col-6">
              <InputText
                id="allergies"
                v-model="nbChildMeals"
                type="text"
                :class="[{ 'p-invalid': errorMessage }, 'w-full']"
                aria-describedby="allergies-help"
                size="small"
                placeholder="Nombre menu enfant"
              />
            </div>
            <div class="field col-12 md:col-6">
              <InputText
                id="allergies"
                v-model="nbAdultMeals"
                type="text"
                :class="[{ 'p-invalid': errorMessage }, 'w-full']"
                aria-describedby="allergies-help"
                size="small"
                placeholder="Nombre menu adulte"
              />
            </div>
            <div class="field col-12">
              <small id="allergies-help"
                >Note: Sauf changement, le menu enfant se compose des pièces cocktail du vin d'honneur, escalope de volaille rotie et pommes
                de terre fondantes / dessert des mariés.</small
              >
            </div>
          </template>
          <div class="field col-12">
